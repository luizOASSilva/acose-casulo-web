...
export default function ActivityDetail({ id, activity, onClose }: ActivityModalProps) {
  const { likes, isLiked, handleLike, likeLabel } = useActivityActions(activity.likes);
        ...
        <div className="flex-1 overflow-y-auto p-6 sm:p-10">
          <div className="mx-auto max-w-4xl">
            <header className="mb-6">
              ...
              <div className="flex flex-wrap gap-3" aria-label="Informações da atividade">
            ...
            <article id="modal-description" className="border-b border-gray-100 pb-8 text-gray-600 leading-relaxed" aria-label={`Descrição de ${activity.title}`}>
              {(activity.content || "")
                .split("\n")
                .filter(p => p.trim())
                .map((p, i) => (
            ...
            <div className="mt-8 flex flex-col items-center justify-between gap-6 pb-4 sm:flex-row sm:pb-0">
              <p className="text-sm font-medium text-gray-500" aria-live="polite" aria-atomic="true">
                <span className="font-bold text-orange-600">{likes}</span> pessoas curtiram
              </p>
              ...
              <button
                ...
                `}
              >
                <Heart size={20} aria-hidden="true" fill={isLiked ? "currentColor" : "none"} className={isLiked ? "animate-pulse" : ""} />
                {likeLabel}
              </button>
            </div>
          </div>
