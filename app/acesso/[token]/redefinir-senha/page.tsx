...
export default async function RedefinirSenha({ params, searchParams }: Props) {
  const { token: accessToken } = await params;
  const { token: resetToken } = await searchParams;

  if (!SECRET_TOKEN || accessToken !== SECRET_TOKEN) {
    notFound();
  }
  ...
  return (
    <main className="min-h-screen">
      <section
        ...
        aria-label="Redefinição de senha da plataforma"
      >
        ...
        <div className="relative flex items-center justify-center overflow-hidden px-6 py-12">
          ...
          <div className="relative z-10 w-full max-w-sm">
            ...
            <ResetPasswordForm accessToken={accessToken} resetToken={resetToken} />
